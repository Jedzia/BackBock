...
namespace Jedzia.BackBock.Tasks
{
    ...
    /// ]]></code>    
    /// </example>  
    [HelpUrl("http://www.msbuildextensionpack.com/help/4.0.4.0/html/f2118b59-554e-d745-5859-126a82b1df81.htm")]
            ...
            if (this.CompressFiles != null)
            {
                using (ZipFile zip = new ZipFile())
                {
                    ...
                    foreach (ITaskItem f in this.CompressFiles)
                    {
                        ...
                        {
                            string location = (f.GetMetadata("RootDir") + f.GetMetadata("Directory")).Replace(this.RemoveRoot.GetMetadata("FullPath"), string.Empty);
                            this.LogTaskMessage(string.Format(CultureInfo.CurrentCulture, "Zipping: {0}", location));
                            zip.AddFile(f.GetMetadata("FullPath"), location);
                        }
                        else
                        {
                            var fullpath = f.GetMetadata("FullPath");
                            this.LogTaskMessage(string.Format(CultureInfo.CurrentCulture, "Zipping: {0}", fullpath));
